...
class DataExtractor():
    ...
    def get_data(self, dataset, level='peak', event_fields=['event_number'], peak_fields=['area','hit_time_std'], hit_fields=[],
        ...
        self.level = level
        self.index_arrays = [np.array([], dtype=np.int32), np.array([], dtype=np.int32)]
        index_names = ['event', 'peak']
        if self.level=='hit':
            self.index_arrays.append(np.array([], dtype=np.int32))
            index_names.append('hit')
